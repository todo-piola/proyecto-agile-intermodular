<!--comentario desde la ramaRafael -->
<!--comentario desde la ramaFranco -->
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>LABUTACASOCIAL</title>
    <link href="css/estilo.css" rel="stylesheet">
    <link href="css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.13.1/font/bootstrap-icons.min.css">
    <link rel="icon" type="image/png" href="img/logo favicon butaca.png">
</head>
<body>
    <div class="container-fluid bg-black">

        <!-- Encabezado con logo -->
        <div class="row" id="encabezado">
            <img src="img/LOGO LABUTACASOCIAL.webp" id="logo-encabezado" alt="logo">
            
        </div>

        <!-- Modal -->
        <div class="modal fade" id="modal">
        <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content bg-dark text-white">
            <div class="modal-header border-warning">
                <h4 class="modal-title">Regístrate en pocos pasos</h4>
                <button type="button" class="btn-close bg-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="form-envio-1">
                    <!--Nombre-->
                    <div class="mb-3 field">
                        <label for="nombreApellido" class="form-label">Nombre y Apellido <span class="asterisco">*</span></label>
                        <input type="text" class="form-control" id="nombreApellido" required>
                        <p id = "errNombre" class="error"></p>
                    </div>
                    <!--Apellido(No tiene validaciones)-->
                    <div class="mb-3 field">
                        <label for="apellido" class="form-label">Apellido <span class="asterisco">*</span></label>
                        <input type="text" class="form-control" id="apellido" required>
                        <p id = "errApellido" class="error"></p>
                    </div>
                    
                    <!--Correo-->
                    <div class="mb-3 field">
                        <label for="correo" class="form-label">Email<span class="asterisco">*</span></label>
                        <input type="text" class="form-control" id="correo" required>
                        <p id = "errCorreo" class="error"></p>
                    </div>
                    
                    <!-- Contraseña -->
                    <div class="mb-3 field">
                        <label for="contrasena" class="form-label">Contraseña<span class="asterisco">*</span>
                            <i class="bi bi-eye-fill"></i>
                        </label>
                        <input type="password" class="form-control" id="contrasena" required>
                        <p id = "errContrasena" class="error"></p>
                    </div>
                    
                    <!-- Confirmar Contraseña -->
                    <div class="mb-3 field">
                        <label for="confirmarContrasena" class="form-label">Confirmar Contraseña<span class="asterisco">*</span></label>
                        <input type="password" class="form-control" id="confirmarContrasena" required>
                        <p id = "errConfirmarContrasena" class="error"></p>
                    </div>
                    
                    <!-- Sexo (Radio buttons) -->
                    <fieldset class="form-check" aria-label="Botones selectores de sexo">
                        <label class="form-check-label">
                            Sexo
                        </label>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="sexo" id="masculino" value="masculino">
                            <label class="form-check-label" for="masculino">
                                Masculino
                            </label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="sexo" id="femenino" value="femenino">
                            <label class="form-check-label" for="femenino">
                                Femenino
                            </label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="sexo" id="prefiero-no-decir" value="prefiero-no-decir">
                            <label class="form-check-label" for="prefiero-no-decir">
                                Prefiero no decir
                            </label>
                        </div>
                    </fieldset>
                    
                    <!-- Fecha de Nacimiento -->
                    <div class="mb-3">
                        <label for="fechaNacimiento" class="form-label">Fecha de Nacimiento</label>
                        <input type="date" class="form-control" id="fechaNacimiento">
                        <p id = "errNacimiento" class="error"></p>
                    </div>
                    
                    <!-- Activar Notificaciones (Checkbox) -->
                    <div class="form-check mb-3">
                        <input class="form-check-input" type="checkbox" id="notificaciones">
                        <label class="form-check-label" for="notificaciones">
                            Activar notificaciones
                        </label>
                    </div>
                    
                    <!-- Recibir Revista Digital (Checkbox) -->
                    <div class="form-check mb-3">
                        <input class="form-check-input" type="checkbox" id="revistaDigital">
                        <label class="form-check-label" for="revistaDigital">
                            Recibir revista digital
                        </label>
                        <p class="exito" id="mensajeExito"></p>
                    </div>
                    <div class="modal-footer border-warning">
                        <button type="submit" class="btn btn-warning" id="btn-envio">Enviar formulario</button>
                    </div>
                </form>
            </div>
        </div>
        </div>
        </div>

        <!-- Hero móvil - Solo visible en pantallas pequeñas -->
        <div class="row d-md-none align-items-center p-5" id="hero-movil" style="background-image: url('img/img-landing-parasite.jpg')">
            <div class="text-start text-white p-1">
                <h2 class="hero-cta">¡Sigue la evolución <br> del cine, sin salir <br> de tu casa!</h2>
                <a href="#" class="btn btn-warning mt-2" data-bs-toggle="modal" data-bs-target="#modal">    <!-- Activador de modal -->
                    <p class="texto-cta">Regístrate ahora</p>
                </a>
            </div>
        </div>

        <!-- Hero escritorio - Visible desde md en adelante -->
        <div class="row d-none d-md-flex align-items-center p-5" id="hero-escritorio" style="background-image: url('img/img-dune-landing.jpeg')">
            <div class="col-5 text-left text-white p-5 div-hero">
                <h1 class="hero-cta">¡Sigue la evolución del cine, sin salir de tu casa!</h1>
                <a href="#" class="btn btn-warning mt-2" data-bs-toggle="modal" data-bs-target="#modal">    <!-- Activador de modal -->
                    <p class="texto-cta">Regístrate ahora</p>
                </a>
            </div>
        </div>

        <!-- Grid de funcionalidades - 6 tarjetas con iconos -->
        <div class="row gap-2 mt-4 d-flex justify-content-center" id="funcionalidades">
            <div class="card col-12 col-md-3 mb-2 mt-2">
                <div class="d-flex justify-content-center">
                    <img src="img/iconos/estrella.png" class="icono-grid" alt="icono estrella">
                </div>
                <div class="card-body">
                  <h4 class="card-title text-center">VALORA</h4>
                  <p class="card-text text-center">tus películas favoritas y expresa tus gustos personales.</p>
                </div>
            </div>
            <div class="card col-12 col-md-3 mb-2 mt-2">
                <div class="d-flex justify-content-center">
                    <img src="img/iconos/teclado.png" class="icono-grid" alt="icono teclado">
                </div>
                <div class="card-body">
                  <h4 class="card-title text-center">ESCRIBE</h4>
                  <p class="card-text text-center">reviews sinceras y comparte tus impresiones mas auténticas.</p>
                </div>
            </div>
            <div class="card col-12 col-md-3 mb-2 mt-2">
                <div class="d-flex justify-content-center">
                    <img src="img/iconos/comparte.png" class="icono-grid" alt="icono comparte">
                </div>
                <div class="card-body">
                  <h4 class="card-title text-center">COMPARTE</h4>
                  <p class="card-text text-center">listas personalizadas con otros usuarios mostrando tus gustos.</p>
                </div>
            </div>
            <div class="card col-12 col-md-3">
                <div class="d-flex justify-content-center">
                    <img src="img/iconos/diamante.png" class=" icono-grid" alt="icono diamante">
                </div>
                <div class="card-body">
                  <h4 class="card-title text-center">SEGUIMIENTO CONTINUO</h4>
                  <p class="card-text text-center">de tus películas favoritas y de las que planeas ver próximamente.</p>
                </div>
            </div>
            <div class="card col-12 col-md-3">
                <div class="d-flex justify-content-center">
                    <img src="img/iconos/corazon.png" class="icono-grid" alt="icono corazón">
                </div>    
                <div class="card-body">
                  <h4 class="card-title text-center">PUNTÚA</h4>
                  <p class="card-text text-center">listas y películas, valora y refleja tus sentimientos y sensaciones.</p>
                </div>
            </div>
            <div class="card col-12 col-md-3 ">
                <div class="d-flex justify-content-center">
                    <img src="img/iconos/lista.png" class="icono-grid" alt="icono lista">
                </div>
                <div class="card-body mb-2 mt-2 tarjeta">
                  <h4 class="card-title text-center">CREA</h4>
                  <p class="card-text text-center">un diario de películas guarda tus sensaciones mas influyentes.</p>
                </div>
            </div>
        </div>

        <!-- Catálogo de películas esperadas -->
        <div class="row p-5 bg-black">
            <div class="col-12 d-md-none">
                <h3 class="text-center text-md-start w-100 text-white peliculas-esperadas-movil">
                    Películas más esperadas de 2026</h3>
            </div>
            <div class="col-12 d-none d-md-flex">
                <h3 class="text-center text-md-start w-100 text-white peliculas-esperadas-escritorio">
                    Películas más esperadas de 2026</h3>
            </div>
            <div class="col-4 col-md-2">
                <div class="bg-dark h-100">
                    <img src="img/poster_CumbresBorrascosas.webp" class="imagen-catalogo w-100 h-100 object-fit-cover" alt="poster cumbres borrascosas">
                </div>
            </div>
            <div class="col-4 col-md-2">
                <div class="bg-dark h-100">
                    <img src="img/poster_MandoGrogu.webp" class="imagen-catalogo w-100 h-100 object-fit-cover" alt="poster mando grogu">
                </div>
            </div>
            <div class="col-4 col-md-2">
                <div class="bg-dark h-100">
                    <img src="img/poster_Odisea_Nolan.webp" class="imagen-catalogo w-100 h-100 object-fit-cover" alt="poster odisea">
                </div>
            </div>
            <div class="col-4 col-md-2">
                <div class="bg-dark h-100">
                    <img src="img/poster_proyect_hailMary.webp" class="imagen-catalogo w-100 h-100 object-fit-cover" alt="poster hail mary">
                </div>
            </div>
            <div class="col-4 col-md-2">
                <div class="bg-dark h-100">
                    <img src="img/poster_theBride.webp" class="imagen-catalogo w-100 h-100 object-fit-cover" alt="poster the bride">
                </div>
            </div>
            <div class="col-4 col-md-2">
                <div class="bg-dark h-100">
                    <img src="img/poster_WeBuryTheDead.webp" class="imagen-catalogo w-100 h-100 object-fit-cover" alt="poster we bury the dead">
                </div>
            </div>
        </div>

        <!-- CTA móvil - Solo visible en pantallas pequeñas -->
        <div class="row d-flex d-md-none align-items-center justify-content-center p-5">
            <div class="text-center text-white ">
                <h3 class="hero-cta">Únete a la comunidad cinéfila del momento y consigue tu medalla distintiva antes que nadie</h3>
                <a href="#" class="btn btn-warning mt-1 justify-content-center" data-bs-toggle="modal" data-bs-target="#modal">  <!-- Activador de modal -->
                    <p class="texto-cta">Empieza tu aventura</p>
                </a>
            </div>
        </div>

        <!-- CTA escritorio - Visible desde md en adelante -->
        <div class="row d-none d-md-flex align-items-center justify-content-center p-5">
            <div class="col-8 text-center text-white p-2">
                <h2 class="hero-cta">Únete a la comunidad cinéfila del momento y  consigue tu medalla distintiva antes que nadie</h2>
                <a href="#" class="btn btn-warning mt-1 justify-content-center" data-bs-toggle="modal" data-bs-target="#modal"> <!-- Activador de modal -->
                    <p class="texto-cta">Empieza tu aventura</p>
                </a>
            </div>
        </div>

        <?php include ("templates/footer.html") ?>
    </div>

    <script src="recursos/bootstrap.bundle.min.js"></script>
    <script type="module" src="./js/main.js"></script>
</body>
</html>